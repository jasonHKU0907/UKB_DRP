...
my_f = pd.read_csv(dpath + 'Results/Results_woHES/DM_full/s04_DM_full.csv')['Features'][:10]
...
for train_idx, test_idx in mykf.split(X, y):
    ...
    #del my_gbm, calibrate

y_pred_prob_df = pd.DataFrame(y_pred_prob_lst).T
y_test_df = pd.DataFrame(y_test_lst).T
y_pred_prob_df.to_csv(dpath + 'Results/Results_woHES/DM_full/pred_prob_cv_df.csv')
y_test_df.to_csv(dpath + 'Results/Results_woHES/DM_full/test_cv_df.csv')

finaloutput = avg_results(results_cv)
finaloutput.to_csv(dpath + 'Results/Results_woHES/DM_full/s05_DM_full.csv')
