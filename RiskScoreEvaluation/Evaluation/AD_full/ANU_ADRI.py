...
mydf_anuadri.columns
mydf_anuadri['anuadri_score'] = mydf_anuadri.iloc[:,3:].sum(axis = 1)
mydf_anuadri['anuadri_score'].value_counts()

#mydf = pd.read_csv(dpath + 'Preprocessed_Data/Preprocessed_Data.csv')
#mydf_anuadri['dementia_status'][mydf['dementia_years']>5] = 0

negative_y = mydf_anuadri['AD_status'].value_counts()[0]
positive_y = mydf_anuadri['AD_status'].value_counts()[1]
...
sens_lst, spec_lst = [], []
for i in range(1, 15):
    fp, tp = mytbl.iloc[:i, :].sum(axis=0)
    tn, fn = mytbl.sum(axis=0) - mytbl.iloc[:i, :].sum(axis=0)
    sens_lst.append(1 - tp / (tp + fn))
    spec_lst.append(tn / (fp + tn))

plt.plot(spec_lst, sens_lst)
'''
